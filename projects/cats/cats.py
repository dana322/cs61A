...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def inner(paragraph): 
        l_paragraph = split(remove_punctuation(lower(paragraph)))
        for t in topic:
            if t in l_paragraph:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    for i in range(min(len(reference_words), len(typed_words))):
        if reference_words[i] == typed_words[i]:
            length += 1
    return 0.0 if len(typed_words) == 0 else (length / len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        if typed_word == word:
            return typed_word
        difflist.append((word, diff_function(typed_word, word, limit)))
    if(min(difflist, key = diffnum))[1] > limit:
        return typed_word
    else:
        return min(difflist, key = diffnum)[0]
    # END PROBLEM 5
...
def sphinx_switches(start, goal, limit):
    ...
    res = sphinx_switches_helper(0, start, goal, 0, limit)
    if  res > limit:
        return limit + 1
    else:
        return res
...
def sphinx_switches_helper(acc, start, goal, i, limit):
    if acc > limit:
        return limit + 1
    if i == min(len(start), len(goal)):
        return acc + abs(len(start) - len(goal))
    if start[i] != goal[i]:
        return sphinx_switches_helper(acc + 1, start, goal, i + 1, limit)
    else:
        return sphinx_switches_helper(acc, start, goal, i + 1, limit)
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    result = []
    # print(len(times_per_player))
    for j in range(len(times_per_player)):
        sub_list = []
        for i in range(1, len(times_per_player[0])):
            sub_list.append(times_per_player[j][i] - times_per_player[j][i - 1])
