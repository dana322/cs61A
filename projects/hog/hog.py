...
def one_heighest(who, score, last_score, running_high):
    """print WHO 's score, return the highest score"""
    if score - last_score > running_high:
        running_high = score - last_score
        print('Player {} has reached a new maximum point gain. {} point(s)!'.format(who, running_high))
    return running_high
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if(who == 1):
            new_height = one_heighest(who, score1, last_score, running_high)
        if(who == 0):
            new_height = one_heighest(who, score0, last_score, running_high)
        return announce_highest(who, score0 if who == 0 else score1, new_height)
    return say
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # return 6  # Replace this statement
    if more_boar(score + piggy_points(opponent_score), opponent_score) or ((more_boar(score, opponent_score) == False) and (piggy_points(opponent_score) >= cutoff)):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
